...
VF_ENV="subclip-video-formatter/.env"
VF_LOCAL_ENV="subclip-video-formatter/.env.local"

# Check if source files exist
if [ ! -f "$API_ENV" ] || [ ! -f "$SE_ENV" ] || [ ! -f "$SG_ENV" ] || [ ! -f "$SM_ENV" ] || [ ! -f "$ST_ENV" ] || [ ! -f "$SI_ENV" ] || [ ! -f "$VF_ENV" ]; then
    echo "Source environment files not found"
    exit 1
fi

rm -r "$API_LOCAL_ENV" "$SE_LOCAL_ENV" "$SG_LOCAL_ENV" "$SM_LOCAL_ENV" "$ST_LOCAL_ENV" "$SI_LOCAL_ENV" "$VF_LOCAL_ENV" 2> /dev/null
...
replace_env_vars "$VF_ENV" "$VF_LOCAL_ENV"
